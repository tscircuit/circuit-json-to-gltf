...
test("decodeBase64Buffer should validate data URI structure", () => {
  ...
  for (const invalidUri of invalidStructures) {
    expect(() => decodeBase64Buffer(invalidUri)).toThrow("Invalid base64 data URI")
  }
})
...
test("loadGLTF should handle negative buffer offsets", () => {
  ...
  return withMockFetch("test://negative-offset.gltf", maliciousGLTF, async () => {
    // Should reject negative offsets with controlled error
    await expect(loadGLTF("test://negative-offset.gltf")).rejects.toThrow(/cannot be negative/)
  })
})
...
test("loadGLTF should handle buffer overrun attempts", () => {
  ...
  return withMockFetch("test://buffer-overrun.gltf", maliciousGLTF, async () => {
    // Should reject buffer overruns with controlled error
    await expect(loadGLTF("test://buffer-overrun.gltf")).rejects.toThrow(/beyond buffer bounds/)
  })
})
...
test("loadGLTF should handle circular references in GLTF structure", () => {
  ...
  const circularGLTF = {
    ...
    buffers: [{
      byteLength: 36,
      uri: "data:application/octet-stream;base64," + btoa(String.fromCharCode(...new Uint8Array(36).fill(65)))
    }]
  }
...
test("loadGLTF should handle malformed binary data gracefully", () => {
  ...
  const malformedBinaryGLTF = {
    ...
    buffers: [{
      ...
      uri: "data:application/octet-stream;base64," + btoa("\x00\xFF\x00\xFF\x00\xFF\x00\xFF\x00\xFF\x00\xFF\x00\xFF\x00\xFF\x00\xFF\x00\xFF\x00\xFF\x00\xFF\x00\xFF\x00\xFF\x00\xFF\x00\xFF\x00\xFF\x00\xFF\x00\xFF\x00\xFF\x00\xFF\x00\xFF")
    }]
  }

  return withMockFetch("test://malformed-binary.gltf", malformedBinaryGLTF, async () => {

...
import { TestGLTFPatterns, TestURLs, createMockFetch, withMockFetch, withMultipleMockFetch } from "../helpers/gltf-test-utils"
...
test("GLTF loader should parse simple GLTF data", async () => {
  await withMockFetch(TestURLs.VALID, TestGLTFPatterns.SIMPLE_TRIANGLE, async () => {
...
test("loadGLTF should handle malformed GLTF gracefully", async () => {
  ...
  await withMockFetch("test://incomplete.gltf", TestGLTFPatterns.EMPTY_GLTF, async () => {
  ...
  await withMockFetch("test://network-error.gltf", TestGLTFPatterns.SIMPLE_TRIANGLE, async () => {
    await expect(loadGLTF("test://network-error.gltf")).rejects.toThrow("Network error")

...
import panelFixture from "../fixtures/panel.json"

test("convert panel fixture to GLB output (Stress test)", async () => {
  const multiplePanelsFixture = Array(3).fill(panelFixture).flat()

  const totalStartMs = performance.now()
  const sceneStartMs = performance.now()
  const scene3d = await convertCircuitJsonTo3D(multiplePanelsFixture as any, {

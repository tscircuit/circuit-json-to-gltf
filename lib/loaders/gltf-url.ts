...
export async function fetchGltfAndConvertToGlb(url: string): Promise<ArrayBuffer> {
  ...
  const totalLength =
    ...
    (binaryBuffer.byteLength > 0 ? (8 + binaryBuffer.byteLength + binaryPadding) : 0); // binary chunk

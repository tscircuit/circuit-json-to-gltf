...
type MeshoptDecoderType = import("meshoptimizer").MeshoptDecoder
...
export async function convertSceneToGLTF(
  ...
  if (!targetScene) {
    ...
    targetScene = scenes[0] ?? root.createScene("Scene")
    root.setDefaultScene(targetScene)
  }

  for (const model of scene.externalModels) {
    await mergeExternalModel(baseDocument, targetScene, io, toolkit, model).catch(
      (error) => {
...
function createInstanceMatrix(model: ExternalModelInstance): Float32Array {

...
An explicit exporter `coordinateTransform` overrides URL/footprinter format
normalization. JSCAD plans retain their fixed native mapping and ignore that
override for both geometry and datum preparation, as in the preceding layer.

This changes the intermediate API contract: `Scene3D`, direct loader results,
and the destination of an explicit `coordinateTransform` are now canonical
Z-up rather than the old Y/Z-swapped scene. Callers supplying scene-space
coordinates or custom loader mappings must adapt those axes. Ordinary Circuit
JSON placement and final glTF coordinates are compared against the preceding
behavioral layer using the same final-output regressions.
...
viewer's runtime URL-fallback policy. `Scene3D.meshType` now agrees with the
unchanged GLB-first loader choice when both GLB and glTF URLs are supplied;
previously its metadata could say glTF while the loaded geometry was GLB.
